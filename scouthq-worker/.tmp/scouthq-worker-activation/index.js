...
// src/index.mjs
var ORG = "org-pack-321";
var enc = new TextEncoder();
var id = /* @__PURE__ */ __name((p) => `${p}_${crypto.randomUUID()}`, "id");
var email = /* @__PURE__ */ __name((e) => String(e || "").trim().toLowerCase(), "email");
...
var page = /* @__PURE__ */ __name((title, body, status = 200) => new Response(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title} | ScoutHQ</title><style>body{margin:0;background:#f4f7fa;color:#082b57;font:16px system-ui}.bar{background:#082b57;color:white;padding:18px 5vw}.bar b{color:#ffc72c;font-size:1.4rem}.wrap{max-width:860px;margin:48px auto;padding:0 24px}.card{background:white;border:1px solid #d8e0e8;border-radius:18px;padding:28px;box-shadow:0 12px 32px #082b5714}button,a.button{display:inline-block;border:0;border-radius:9px;padding:12px 18px;background:#ffc72c;color:#082b57;font-weight:800;text-decoration:none}input{display:block;width:min(100%,420px);padding:12px;margin:8px 0 18px}.muted{color:#52667b}.danger{color:#9c1c1c}</style></head><body><header class="bar"><b>ScoutHQ</b> <span>Pack 321 staging</span></header><main class="wrap">${body}</main></body></html>`, { status, headers: { "content-type": "text/html;charset=utf-8", "cache-control": "no-store" } }), "page");
var hash = /* @__PURE__ */ __name(async (value) => [...new Uint8Array(await crypto.subtle.digest("SHA-256", enc.encode(value)))].map((x) => x.toString(16).padStart(2, "0")).join(""), "hash");
var cookie = /* @__PURE__ */ __name((request, name) => request.headers.get("cookie")?.split(";").map((x) => x.trim()).find((x) => x.startsWith(`${name}=`))?.slice(name.length + 1), "cookie");
var accessIdentity = /* @__PURE__ */ __name(async (request, env) => {
  ...
  if (!payload.sub || !payload.email) throw Object.assign(new Error(), { status: 401 });
  return { sub: String(payload.sub), email: email(payload.email), name: String(payload.name || payload.email) };
}, "accessIdentity");
var upsert = /* @__PURE__ */ __name(async (db, a) => {
  let u = await db.prepare("SELECT u.* FROM auth_identities ai JOIN users u ON u.id=ai.user_id WHERE ai.provider=? AND ai.provider_subject=?").bind("cloudflare-access", a.sub).first();
  if (!u) {
    u = await db.prepare("SELECT * FROM users WHERE primary_email=? AND email_verified=1").bind(a.email).first();
    const uid = u?.id || id("usr");
    if (!u) await db.prepare("INSERT INTO users(id,primary_email,email_verified,display_name,account_status,onboarding_status) VALUES(?,?,?,?, 'active','new')").bind(uid, a.email, 1, a.name).run();
    await db.prepare("INSERT INTO auth_identities(id,user_id,provider,provider_subject,provider_email,verified,last_authenticated_at) VALUES(?,?,?,?,?,1,CURRENT_TIMESTAMP)").bind(id("aid"), uid, "cloudflare-access", a.sub, a.email).run();
    u = await db.prepare("SELECT * FROM users WHERE id=?").bind(uid).first();
  }
  await db.prepare("UPDATE users SET last_sign_in_at=CURRENT_TIMESTAMP WHERE id=?").bind(u.id).run();
  return u;
}, "upsert");
var session = /* @__PURE__ */ __name(async (request, db) => {
  const token = cookie(request, "scouthq_session");
  if (!token) return null;
  return db.prepare("SELECT u.* FROM application_sessions s JOIN users u ON u.id=s.user_id WHERE s.id_hash=? AND s.revoked_at IS NULL AND s.expires_at>CURRENT_TIMESTAMP AND u.account_status='active'").bind(await hash(token)).first();
}, "session");
var audit = /* @__PURE__ */ __name((db, actor, action, type, target, details = {}) => db.prepare("INSERT INTO audit_events(id,organization_id,actor_user_id,action,target_type,target_id,details_json) VALUES(?,?,?,?,?,?,?)").bind(id("aud"), ORG, actor, action, type, target, JSON.stringify(details)).run(), "audit");
var requireSession = /* @__PURE__ */ __name(async (request, env) => {
...
var index_default = { async fetch(request, env) {
  try {
    const url = new URL(request.url), db = env.IDENTITY_DB;
    if (url.pathname === "/health") return json({ ok: true, environment: env.ENVIRONMENT });
    if (url.pathname === "/auth/session") {
      const a = await accessIdentity(request, env), u2 = await upsert(db, a), token = crypto.randomUUID() + crypto.randomUUID(), ttl = Number(env.SESSION_TTL_SECONDS || 28800);
      await db.prepare("INSERT INTO application_sessions(id_hash,user_id,expires_at) VALUES(?,?,datetime('now',?))").bind(await hash(token), u2.id, `+${ttl} seconds`).run();
      await audit(db, u2.id, "session.created", "user", u2.id);
      return new Response(null, { status: 302, headers: { location: "/", "set-cookie": `scouthq_session=${token}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${ttl}`, "cache-control": "no-store" } });
    }
    if (url.pathname === "/logout") {
      const token = cookie(request, "scouthq_session");
      if (token) await db.prepare("UPDATE application_sessions SET revoked_at=CURRENT_TIMESTAMP WHERE id_hash=?").bind(await hash(token)).run();
      return new Response(null, { status: 302, headers: { location: "/", "set-cookie": "scouthq_session=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0" } });
    }
    if (url.pathname === "/") {
      const u2 = await session(request, db);
      if (!u2) return page("Sign in", '<section class="card"><h1>Welcome to ScoutHQ</h1><p>Sign in with Google to continue or begin first-time Pack access.</p><a class="button" href="/auth/session">Continue with Google</a></section>');
      const c2 = await buildAuthorizationContext(db, u2.id, ORG);
      if (!c2.membership) return page("Onboarding", `<section class="card"><h1>Let\u2019s set up your household</h1><p class="muted">Signed in as ${u2.display_name}</p><form id="f"><label>Household reference name<input name="householdName" required maxlength="80"></label><button>Save and request Pack access</button></form><p id="s"></p><script>f.onsubmit=async e=>{e.preventDefault();let d=Object.fromEntries(new FormData(f));await fetch('/api/onboarding',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(d)});await fetch('/api/access-requests',{method:'POST'});location.reload()}<\/script><p><a href="/logout">Sign out</a></p></section>`);
      if (c2.membership.status === "pending") return page("Pending access", '<section class="card"><h1>Your Pack 321 request is pending</h1><p>A Pack leader will review your request inside ScoutHQ.</p><a href="/logout">Sign out</a></section>');
      if (c2.membership.status !== "approved") return page("Access unavailable", '<section class="card"><h1>Access unavailable</h1><p class="danger">Your Pack membership is not active.</p><a href="/logout">Sign out</a></section>', 403);
      const lead = c2.permissions.includes("people.read");
      return page("My Day", `<section class="card"><p class="muted">${lead ? "Leadership" : "Parent / Guardian"} experience</p><h1>My Day</h1><p>Welcome, ${u2.display_name}.</p>${lead ? '<p><a class="button" href="/leadership/approvals">Review access requests</a></p>' : "<p>Your family-safe ScoutHQ workspace is ready.</p>"}<a href="/logout">Sign out</a></section>`);
    }
    const u = await requireSession(request, env), c = await buildAuthorizationContext(db, u.id, ORG);
    if (url.pathname === "/api/onboarding" && request.method === "POST") {
      const b = await request.json();
      let h = await db.prepare("SELECT h.* FROM households h JOIN household_members hm ON hm.household_id=h.id WHERE hm.user_id=? AND h.organization_id=?").bind(u.id, ORG).first();
      if (!h) {
        const hid = id("hh");
        await db.batch([db.prepare("INSERT INTO households(id,organization_id,display_name,status) VALUES(?,?,?,'preliminary')").bind(hid, ORG, String(b.householdName || "New household").slice(0, 80)), db.prepare("INSERT INTO household_members(id,household_id,user_id,relationship,primary_guardian) VALUES(?,?,?,'guardian',1)").bind(id("hm"), hid, u.id)]);
        h = { id: hid };
      }
      await db.prepare("INSERT INTO onboarding_sessions(id,user_id,organization_id,household_id,current_step,status) VALUES(?,?,?,?,?,'in_progress') ON CONFLICT(user_id,status) DO UPDATE SET current_step=excluded.current_step,updated_at=CURRENT_TIMESTAMP").bind(id("onb"), u.id, ORG, h.id, "access_request").run();
      return json({ ok: true, householdId: h.id }, 201);
    }
    if (url.pathname === "/api/access-requests" && request.method === "POST") {
      const rid = id("req");
      await db.batch([db.prepare("INSERT INTO organization_memberships(id,user_id,organization_id,status) VALUES(?,?,?,'pending') ON CONFLICT(user_id,organization_id) DO UPDATE SET status='pending'").bind(id("mem"), u.id, ORG), db.prepare("INSERT INTO access_requests(id,requester_user_id,organization_id,requested_relationship,status) VALUES(?,?,?,'parent_guardian','pending')").bind(rid, u.id, ORG)]);
      await audit(db, u.id, "access.requested", "access_request", rid);
    ...
    if (url.pathname === "/leadership/approvals") {
      ...
      const rows = (await db.prepare("SELECT ar.id,u.display_name,u.primary_email FROM access_requests ar JOIN users u ON u.id=ar.requester_user_id WHERE ar.organization_id=? AND ar.status='pending'").bind(ORG).all()).results;
      return page("Approvals", `<section class="card"><h1>Pending access requests</h1>${rows.map((r) => `<article><h2>${r.display_name}</h2><p>${r.primary_email}</p><button onclick="fetch('/api/leadership/access-requests/${r.id}/approve',{method:'POST'}).then(()=>location.reload())">Approve Parent access</button></article>`).join("") || "<p>No pending requests.</p>"}<p><a href="/">My Day</a></p></section>`);
    }
    const m = url.pathname.match(/^\/api\/leadership\/access-requests\/([^/]+)\/approve$/);
    if (m && request.method === "POST") {
      requirePermission(c, "access_requests.review");
      const r = await db.prepare("SELECT * FROM access_requests WHERE id=? AND status='pending'").bind(m[1]).first();
      if (!r) return json({ error: "Not found" }, 404);
      const mem = await db.prepare("SELECT id FROM organization_memberships WHERE user_id=? AND organization_id=?").bind(r.requester_user_id, ORG).first();
      await db.batch([db.prepare("UPDATE access_requests SET status='approved',reviewed_by_user_id=?,reviewed_at=CURRENT_TIMESTAMP WHERE id=?").bind(u.id, r.id), db.prepare("UPDATE organization_memberships SET status='approved',approved_at=CURRENT_TIMESTAMP WHERE id=?").bind(mem.id), db.prepare("INSERT INTO membership_roles(id,membership_id,role_id,assigned_by_user_id) VALUES(?,?,?,?)").bind(id("mr"), mem.id, "role-parent", u.id)]);
      await audit(db, u.id, "access.approved", "access_request", r.id);
      return json({ ok: true });
    }
    const hm = url.pathname.match(/^\/api\/households\/([^/]+)$/);
    if (hm) {
      requireOwnHousehold(c, hm[1]);
      return json(await db.prepare("SELECT id,display_name,status FROM households WHERE id=? AND organization_id=?").bind(hm[1], ORG).first());
    }
    return json({ error: "Not found" }, 404);
  } catch (e) {
    const s = e.status || 500;
    if (s === 500) console.error(JSON.stringify({ event: "request_error", message: e.message }));
    return json({ error: s === 401 ? "Authentication required" : s === 403 ? "Forbidden" : "Request failed" }, s);
  }
} };
export {
