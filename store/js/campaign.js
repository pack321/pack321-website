(function(){
  ...
  const campaignLabel=campaign=>{
    const labels={
      'candy-builder':'Candy Bar Fundraiser',
  ...
  document.addEventListener('DOMContentLoaded',async()=>{
    ...
    try{
      ...
      host.innerHTML=`<section class="campaign-hero campaign-hero--fundraiser"><div class="campaign-hero__content"><p class="campaign-hero__eyebrow fundraiser-eyebrow">Current Fundraiser</p><h1 class="campaign-hero__title fundraiser-hero-title">${StoreUtils.escapeHtml(standardizedHeroTitle)}</h1>${vendorLine(campaign)}<p class="campaign-hero__description">${StoreUtils.escapeHtml(campaign.story||campaign.description)}</p><div class="campaign-hero__actions"><a class="button gold" href="#products">Shop Products</a><a class="button light" href="#story">Why It Matters</a></div></div><div class="campaign-hero__media"><img src="${StoreUtils.escapeHtml(media.hero.src)}" alt="${StoreUtils.escapeHtml(media.hero.alt)}" width="1400" height="800" fetchpriority="high"></div></section>
      ...
      <section class="page-section soft-section" id="products"><div class="wrap"><div class="section-heading"><p class="eyebrow">Campaign products</p><h2>Choose your way to support</h2></div>${window.PackCampaignBuilders.render(campaign,campaignProducts,symbols)}</div></section>
